...
public class Player extends Entity {
    ...
    // Reload sprites based on equipped weapon (handles movement override and attack
    // folders)
    ...
    private void reloadSpritesBasedOnWeapon() {
        ...
        if (attackFolder != null) {
            if (weapon != null && weapon.getClass().getSimpleName().equals("Hanger")) {
                ...
                for (int i = 0; i < 6; i++) {
                    attackDownFrames[i] = loadImageFromFile(
                            basePath + attackFolder + "/Sword_Walk_Attack_front" + (i + 1) + ".png");
    ...
    private void handleAttackInput() {
        // replace consumeAttackTap with whatever KeyHandler uses for attack (assumes
        // method exists)
    ...
    public void setChanneling(boolean ch) {
        channeling = ch;
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        if (attacking && weapon != null && attackFrameIndex < 6 &&
            ...
            switch (direction) {
                case "up":
                    image = (attackFrameIndex < attackUpFrames.length && attackUpFrames[attackFrameIndex] != null)
                            ? attackUpFrames[attackFrameIndex]
                            : null;
                    break;
                case "down":
                    image = (attackFrameIndex < attackDownFrames.length && attackDownFrames[attackFrameIndex] != null)
                            ? attackDownFrames[attackFrameIndex]
                            : null;
                    break;
                case "left":
                    image = (attackFrameIndex < attackLeftFrames.length && attackLeftFrames[attackFrameIndex] != null)
                            ? attackLeftFrames[attackFrameIndex]
                            : null;
                    break;
                case "right":
                    image = (attackFrameIndex < attackRightFrames.length && attackRightFrames[attackFrameIndex] != null)
                            ? attackRightFrames[attackFrameIndex]
                            : null;
                    break;
            }
